...
public class AnimationView extends View
{
    ...
    {
        PULL_HEIGHT = (int) TypedValue
                .applyDimension(TypedValue.COMPLEX_UNIT_DIP, 100,
                        context.getResources().getDisplayMetrics());
        PULL_DELTA = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50,
                context.getResources().getDisplayMetrics());
        mWidthOffset = 0.5f;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        {
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(PULL_DELTA + PULL_HEIGHT,
    ...
    private void drawDrag(Canvas canvas)
    {
        ...
        mPath.quadTo(mWidthOffset * mWidth,
                PULL_HEIGHT + (mHeight - PULL_HEIGHT) * 2,
                mWidth, PULL_HEIGHT);
    ...
    private void drawPopBall(Canvas canvas)
    {
        ...
        canvas.drawArc(new RectF(mWidth / 2 - mRadius, cirCenY - mRadius,
                        mWidth / 2 + mRadius, cirCenY + mRadius),
    ...
    private void drawRefreshing(Canvas canvas)
    {
        ...
        canvas.drawArc(
                new RectF(mWidth / 2 - outerR, innerY - outerR, mWidth / 2 + outerR,
                        innerY + outerR),
    ...
    private void drawDone(Canvas canvas)
    {
        ...
        if (getDoneRatio() >= 0.3 && getDoneRatio() < 0.7)
        {
            ...
            int curCentY = (int) (startCentY +
                    (PULL_DELTA / 2 + mRadius * 2) * fraction);
    ...
    private float getPopRatio()
    {
        ...
        float ratio =
                (System.currentTimeMillis() - mPopStart) / (float) POP_BALL_DUR;
        return Math.min(ratio, 1);
    }
