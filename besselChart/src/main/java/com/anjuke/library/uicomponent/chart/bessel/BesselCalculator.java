...
class BesselCalculator
{
    ...
    private void computeVertcalAxisInfo()
    {
        ...
        float x = verticalTextRect.width() *
                (0.5f + style.getVerticalLabelTextPaddingRate());
        ...
        yAxisWidth = (int) (verticalTextRect.width() *
                (1 + style.getVerticalLabelTextPaddingRate() * 2));
    ...
    private void computeTitlesInfo()
    {
        ...
        float stepX = (width - style.getHorizontalTitlePaddingLeft() -
                style.getHorizontalTitlePaddingRight()) / count;
    ...
    private void computeSeriesCoordinate()
    {
        ...
        for (Series series : data.getSeriesList())
        {
            ...
            for (int i = 0; i < points.size(); i++)
            {
                ...
                float ratio = (point.valueY - data.getMinValueY()) /
                ...
                if (marker != null && marker.getPoint().valueX == point.valueX)
                {
                    ...
                    ratio = (markerPoint.valueY - data.getMinValueY()) /
                            (float) (data.getMaxValueY() - data.getMinValueY());
                    markerPoint.y = maxCoordinateY -
    ...
    private void computeGridPoints()
    {
        ...
        for (Series series : seriesList)
        {
            for (Point point : series.getPoints())
            {
                ...
                if (gridPoints[index] == null ||
                        gridPoints[index].valueY < point.valueY)
                {
     ...
     * 计算非单调情况的贝塞尔结点
     */
    private void computeUnMonotonePoints(int i, List<Point> points,
                                         List<Point> besselPoints)
    {
        if (i == 0)
        {
     ...
     * 计算单调情况的贝塞尔结点
     */
    private void computeMonotonePoints(int i, List<Point> points,
                                       List<Point> besselPoints)
    {
        Point p0 = points.get(i - 1);
        Point p1 = points.get(i);
