...
public class HorizontalLegendView extends View
{
    ...
    public HorizontalLegendView(Context context, List<Title> titles,
                                ChartStyle style, BesselCalculator calculator)
    {
        super(context);
        this.titles = titles;
        this.style = style;
        ...
        for (Title title : titles)
        {
            ...
            if (title instanceof Marker)
            {
                ...
                canvas.drawBitmap(marker.getBitmap(), null,
                        marker.updateRect(title.circleX, title.circleY,
                                title.radius * 2, title.radius * 2), paint);
            }
            else
            {
