...
public class BesselChart extends LinearLayout
{
    ...
    public BesselChart(Context context, AttributeSet attrs)
    {
        ...
        position = a.getInt(R.styleable.ChartStyle_verticalAxisPosition,
                VerticalAxisView.POSITION_RIGHT);
        a.recycle();
        init();
    }
    ...
    private void init()
    {
        ...
        verticalAxis = new VerticalAxisView(getContext(), data.getYLabels(), style,
                calculator);
        horizontalLegend = new HorizontalLegendView(getContext(), data.getTitles(),
                style, calculator);
        besselChartLayout.setOrientation(LinearLayout.HORIZONTAL);
        verticalAxis.setPosition(position);
        besselChartLayout.addView(verticalAxis,
                new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,
                        LayoutParams.WRAP_CONTENT));
        if (position == VerticalAxisView.POSITION_LEFT)
        {
            besselChartLayout.addView(besselChartView,
                    new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                            LayoutParams.WRAP_CONTENT, 1));
        }
        else
        {
            besselChartLayout.addView(besselChartView, 0,
                    new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                            LayoutParams.WRAP_CONTENT, 1));
        }
        addView(besselChartLayout,
                new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                        LayoutParams.WRAP_CONTENT));
        addView(horizontalLegend,
                new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                        LayoutParams.WRAP_CONTENT));
    }
